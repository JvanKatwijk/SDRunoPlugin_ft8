...
const uint8_t FT8_CHECKSUMS [FT8_CHECKSUM_INDICES][7] = {
...
// The numbers indicate which three LDPC parity checks
// (rows in Nm) refer to the codeword bit. 
// 1-origin.
const uint8_t FT8_PARITY_CHECKS [FT8_LDPC_BITS][3] = {
    { 16, 45, 73 },
    { 25, 51, 62 },
    { 33, 58, 78 },
    { 1, 44, 45 },
...
const uint8_t kFTX_LDPC_Num_rows [FT8_CHECKSUM_INDICES] = {
...
//float m [FT8_CHECKSUM_INDICES][FT8_LDPC_BITS]; // ~60 kB
//float e [FT8_CHECKSUM_INDICES][FT8_LDPC_BITS]; // ~60 kB
float **m  = new float *[FT8_CHECKSUM_INDICES];
float **e  = new float *[FT8_CHECKSUM_INDICES]; // ~60 kB

	for (int i = 0; i < FT8_CHECKSUM_INDICES; i ++) {
	   m [i] = new float [FT8_LDPC_BITS];
	   e [i] = new float [FT8_LDPC_BITS];
	}

int min_errors = FT8_CHECKSUM_INDICES;
//
//	we maken M kopien van het codewoord
	for (int j = 0; j < FT8_CHECKSUM_INDICES; j++) {
...
//	prepare for decoding
	   for (int j = 0; j < FT8_CHECKSUM_INDICES; j++) {
	      for (int ii1 = 0; ii1 < kFTX_LDPC_Num_rows [j]; ii1++) {
	         int i1 = FT8_CHECKSUMS [j] [ii1] - 1;
	         float a = 1.0f;
	         for (int ii2 = 0; ii2 < kFTX_LDPC_Num_rows [j]; ii2++) {
	            int i2 = FT8_CHECKSUMS [j][ii2] - 1;
	            if (i2 != i1) {
	               a *= tanh (-m [j][i2] / 2.0f);
...
//
//	feitelijke detectie loop gaat over N
	   for (int i = 0; i < FT8_LDPC_BITS; i++) {
	      float l = codeword [i];
	      for (int j = 0; j < 3; j++)
	         l += e [FT8_PARITY_CHECKS [i][j] - 1][i];
	      plain [i] = (l > 0) ? 1 : 0;
	   }
...
//	Update the current best result
	      ...
	      for (int ji1 = 0; ji1 < 3; ji1++) {
	         int j1 = FT8_PARITY_CHECKS [i][ji1] - 1;
	         float l = codeword[i];
	         for (int ji2 = 0; ji2 < 3; ji2++) {
	            if (ji1 != ji2) {
	               int j2 = FT8_PARITY_CHECKS [i][ji2] - 1;
	               l += e[j2] [i];
	            }
	         }
	         m [j1] [i] = l;
...
void	ldpc::bp_decode (float codeword [], int max_iters,
	                              uint8_t plain [], int* ok) {
float tov [FT8_LDPC_BITS][3];
float toc [FT8_LDPC_BITS][7];
//float toc [FT8_CHECKSUM_INDICES][7];

int min_errors = FT8_CHECKSUM_INDICES;

//	initialize message data
	for (int n = 0; n < FT8_LDPC_BITS; n++) {
...
//	if message converged to all-zeros, which is prohibited, break
	   if (plain_sum == 0) {
	      break;
	   }

//	Check to see if we have an errorless codeword
//	(check before we do any iter)
	   int errors = ldpc_check (plain);
	   if (errors == 0)
	      break;
//	if we have a better guess - update the result
	   if (errors < min_errors) {
	      min_errors = errors;
	   }

//	Send messages from bits to check nodes
	   for (int m = 0; m < FT8_CHECKSUM_INDICES; m++) {
	      for (int n_idx = 0; n_idx < kFTX_LDPC_Num_rows [m]; n_idx ++) {
	         int n = FT8_PARITY_CHECKS [m][n_idx] - 1;
//	for each (n, m)
	         float Tnm = codeword [n];
	         for (int m_idx = 0; m_idx < 3; m_idx ++) {
	            if ((FT8_PARITY_CHECKS [n][m_idx] - 1) != m) {
	               Tnm += tov [n][m_idx];
	            }
	         }
...
//	send messages from check nodes to variable nodes
	   for (int n = 0; n < FT8_LDPC_BITS; n ++) {
	      for (int m_idx = 0; m_idx < 3; m_idx ++) {
	         int m = FT8_PARITY_CHECKS [n][m_idx] - 1;
//	for each (n, m)
	         float Tmn = 1.0f;
	         for (int n_idx = 0; n_idx < kFTX_LDPC_Num_rows [m]; n_idx ++) {
	            if ((FT8_CHECKSUMS [m][n_idx] - 1) != n) {
	               Tmn *= toc [m][n_idx];
	            }
	         }
...
int errors = 0;
	...
	for (int m = 0; m < FT8_CHECKSUM_INDICES; m ++) {
	   ...
	   for (int i = 0; i < kFTX_LDPC_Num_rows [m]; ++i) {
	      x ^= codeword [FT8_CHECKSUMS [m][i] - 1];
	   }
	   if (x != 0) {
	      errors ++;
	   }
