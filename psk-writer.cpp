...
//	pattern for sender callsign
  0x00, 0x02, 0x00, 0x44, 0x99, 0x93, 0x00, 0x08,
	...
	reporterWriter::reporterWriter  (SDRunoPlugin_ft8Ui *m_form) {
struct hostent *host;
	...
	this	-> programName	= "SDRunoPlugin_ft8";
	sequence		= 1;
	reporterOK		= false;
...
//
	...
	std::string homeLine = homeCall + " " + homeGrid + " " + antenna;
	m_form ->  ft8_textBlock (homeLine);
	reporterOK	= true;
}
...
int	reporterWriter::sendMessages	() {
	...
	for (int i = 0; i < messageStack. size (); i ++) {
	   ...
	   if (m. showMessage) {
	      std::string ss = m. call + "," +
	                       std::to_string (m. freq) + "," +
	                       std::to_string (m. snr)  + "." +
	                       "FT8," +
	                       m. grid + "," +
	                       std::to_string (m. seconds);
	      m_form ->  ft8_textBlock (ss);
	   }
	}
